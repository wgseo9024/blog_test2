...
- 네이버 글쓰기 페이지 접근
- 제목과 본문 입력 필드 선택자
- 승인된 처리 이미지 1장 이상(최대 4장 사용)과 본문 문장 블록 7개 이상
- mainFrame의 이미지 버튼 또는 file input과 임시저장 버튼

Dry Run은 lease를 획득하지 않으며 이미지를 다운로드·업로드하거나 내용을 입력하고 임시저장·공개 발행하지 않습니다.
...
인증 API로 받은 이미지 4장과 본문 문장 7개를 이미지-문장 순서로 입력하고 태그를 추가한 뒤 **임시저장 버튼만** 누릅니다. 성공 신호를 확인하면 서버에
`released` 결과를 기록하여 초안을 다시 `queued` 상태로 돌려놓습니다. 브라우저에서
임시저장 글을 확인한 뒤 Enter를 누르면 종료합니다. 다운로드 임시 파일은 성공과 실패 모두 자동 삭제됩니다.
