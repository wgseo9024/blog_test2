...
test("대기열 API는 승인 이미지를 정렬해 최대 4장만 반환한다",async()=>{const src=await readFile(new URL("../../functions/api/publisher/queued.js",import.meta.url),"utf8");assert.match(src,/approved_for_use = 1/);assert.match(src,/rights_status = 'approved'/);assert.match(src,/ORDER BY ai\.sort_order ASC, ai\.id ASC LIMIT 4/);assert.doesNotMatch(src,/images[^\n]*length[^\n]*[!=]==?\s*4/);});
