...
const draft = (imageCount = 4, blockCount = 6) => ({
...
test("승인 이미지 0장이면 failed로 중단한다", () => {
  assert.throws(() => validateDraftAssets(draft(0, 6)), (error) => error instanceof PublisherStageError && error.result === "failed" && error.message === "승인된 처리 이미지가 없습니다.");
});

for (const imageCount of [1, 2, 3, 4]) test(`승인 이미지 ${imageCount}장이면 진행한다`, () => {
  assert.equal(validateDraftAssets(draft(imageCount, 6)).images.length, imageCount);
});

test("승인 이미지 5장이면 정렬된 앞의 4장만 사용한다", () => {
  assert.deepEqual(validateDraftAssets(draft(5, 6)).images.map(({ id }) => id), [1, 2, 3, 4]);
});

test("body_blocks가 정확히 6개가 아니면 failed로 중단한다", () => {
  assert.throws(() => validateDraftAssets(draft(4, 5)), (error) => error instanceof PublisherStageError && error.result === "failed");
});
...
test("이미지 아래 텍스트 블록 생성 실패는 failed 오류를 낸다", async () => {
  ...
  await assert.rejects(() => ensureTextBlockAfterLastImage(frame, { beforeTextCount: 1, timeout: 5 }),
    (error) => error instanceof PublisherStageError && error.result === "failed" && /이미지 후 텍스트 블록 생성 실패/.test(error.message));
});
