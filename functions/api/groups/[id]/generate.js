...
본문은 기사와 Fact에서 확인된 내용 안에서만 작성하며 없는 내용을 임의로 추가하지 않는다. 원문을 그대로 베끼지 않고 자연스럽게 재구성하고, 딱딱한 기사 요약이 아닌 읽기 쉬운 연예뉴스 전문 블로그 톤으로 쓴다. 정확히 7개의 완결된 한 문장 bodyBlocks를 출력하고, 7문장 합계는 공백 포함 반드시 600~800자로 맞춘다. 출력 전에 글자 수를 직접 확인한다. 별도 소제목은 넣지 않고 모바일에서 읽기 쉬운 길이로 쓴다.
...
export const VALIDATOR_PROMPT = "기사와 Fact에 없는 인물·사건·반응, 과장하거나 자극적인 제목, 비난·단정, 원문 과도 복제, 소제목 사용, 정확히 7 blocks, 중복 없는 10 tags, 본문 공백 포함 600~800자를 엄격히 검사하라. 글자 수 조건은 반드시 지켜야 한다.";
export { schemas };
