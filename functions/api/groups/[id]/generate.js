...
본문은 기사와 Fact에서 확인된 내용 안에서만 작성하며 없는 내용을 임의로 추가하지 않는다. 원문을 그대로 베끼지 않고 자연스럽게 재구성하고, 딱딱한 기사 요약이 아닌 읽기 쉬운 연예뉴스 전문 블로그 톤으로 쓴다. 정확히 6개의 완결된 한 문장 bodyBlocks를 출력하고, 각 문장은 공백 포함 117~133자를 목표로 작성하되 절대로 90자 미만이 되어서는 안 되며, 6문장 합계는 공백 포함 반드시 700~800자로 맞춘다. 출력 전에 각 문장의 실제 글자 수와 전체 글자 수를 직접 세어 조건을 벗어나면 스스로 늘리거나 줄인다. 별도 소제목은 넣지 않고 모바일에서 읽기 쉬운 길이로 쓴다.
...
export const VALIDATOR_PROMPT = "기사와 Fact에 없는 인물·사건·반응, 과장하거나 자극적인 제목, 비난·단정, 원문 과도 복제 여부를 검사하라. 소제목은 금지이므로 소제목이 있으면 오류이고, 소제목이 없는 것은 정상이다. 정확히 6 blocks, 각 block 90자 이상, 중복 없는 10 tags, 본문 공백 포함 700~800자를 엄격히 검사하라. 문장별 글자 수와 전체 글자 수 조건을 반드시 지켜야 한다.";
export { schemas };
...
export async function generateGroupDraft(env, rawId, options = {}) {
  ...
  const allRows = (await env.DB.prepare(`SELECT a.* FROM article_group_items i JOIN articles a ON a.id=i.article_id WHERE i.group_id=? AND COALESCE(a.is_advertisement,0)=0 ORDER BY COALESCE(a.published_at,a.created_at) DESC`).bind(groupId).all()).results || [];
  const seenSources = new Set();
  const rows = allRows.filter((article) => { const key = clean(article.source, 200).toLowerCase(); if (seenSources.has(key)) return false; seenSources.add(key); return true; }).slice(0, 3);
  ...
  try {
    ...
    if (!local.valid) throw new Error(`DRAFT_FORMAT_INVALID:${local.issues.map((issue) => issue.code).join(",")}`);
    const validationStatus = "review_required";
    const status = "review";
    ...
    const code = error.message === "OPENAI_API_ERROR" ? "OPENAI_API_ERROR" : "DRAFT_PROCESSING_ERROR";
    const detail = [error.status ? `HTTP ${error.status}` : "", error.detail || error.message].filter(Boolean).join(" · ");
    return fail(`초안 생성 또는 검증에 실패했습니다${detail ? `: ${detail}` : "."}`, 502, code);
  }
}
