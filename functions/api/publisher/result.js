...
export async function onRequestPost({ request, env }) {
  ...
  if (!Number.isSafeInteger(draftId) || !lease || !["published", "failed", "released", "login_required"].includes(outcome)) {
    return json({ success: false, error: { message: "초안 id, lease, 결과 값을 확인해 주세요." } }, 400);
  }
  const status = ["released", "login_required"].includes(outcome) ? "queued" : outcome;
  const changed = await env.DB.prepare(`UPDATE drafts SET status = ?, lease_token = NULL, lease_expires_at = NULL,
    ...
    publish_error = CASE WHEN ? IN ('failed','login_required') THEN ? ELSE NULL END,
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO draft_publish_events (draft_id, result, message) VALUES (?, ?, ?)").bind(draftId, outcome === "login_required" ? "released" : outcome, message || null),
    env.DB.prepare("INSERT INTO publish_logs (draft_id, action, message) VALUES (?, ?, ?)").bind(draftId, ["released","login_required"].includes(outcome) ? "queued" : outcome, message || null),
  ]);
