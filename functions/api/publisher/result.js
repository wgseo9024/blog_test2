...
export async function onRequestPost({ request, env }) {
  ...
  if (!Number.isSafeInteger(draftId) || !lease || !["published", "failed", "released"].includes(outcome)) {
    return json({ success: false, error: { message: "초안 id, lease, 결과 값을 확인해 주세요." } }, 400);
  }
  const status = outcome === "released" ? "queued" : outcome;
  const changed = await env.DB.prepare(`UPDATE drafts SET status = ?, lease_token = NULL, lease_expires_at = NULL,
    ...
    publish_error = CASE WHEN ? = 'failed' THEN ? ELSE NULL END, updated_at = CURRENT_TIMESTAMP
    WHERE id = ? AND lease_token = ?`).bind(status, outcome, outcome, message || null, draftId, lease).run();
  if (!changed.meta?.changes) return json({ success: false, error: { message: "유효한 발행 lease를 찾을 수 없습니다." } }, 409);
  await env.DB.batch([
    env.DB.prepare("INSERT INTO draft_publish_events (draft_id, result, message) VALUES (?, ?, ?)").bind(draftId, outcome, message || null),
    env.DB.prepare("INSERT INTO publish_logs (draft_id, action, message) VALUES (?, ?, ?)").bind(draftId, outcome === "released" ? "queued" : outcome, message || null),
  ]);
