...
export async function onRequestGet({request,env,params}) { if(!publisherAuthorized(request,env))return fail("이미지 조회 권한이 없습니다.",env.PUBLISHER_TOKEN?401:503);const id=validId(params.id);if(!id)return fail("올바른 이미지 id가 아닙니다.");const row=await env.DB.prepare("SELECT processed_r2_key FROM article_images WHERE id=? AND approved_for_use=1 AND rights_status='approved' AND processed_r2_key IS NOT NULL").bind(id).first();if(!row?.processed_r2_key)return fail("처리 이미지를 찾을 수 없습니다.",404);const object=await env.IMAGES_BUCKET.get(row.processed_r2_key);if(!object)return fail("R2 객체를 찾을 수 없습니다.",404);return new Response(object.body,{headers:{"Content-Type":object.httpMetadata?.contentType||"image/jpeg","Content-Length":String(object.size),"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff"}}); }
