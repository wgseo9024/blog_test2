...
- 본문만 공백 포함 반드시 600~800자로 맞추고, 모바일에서 읽기 쉽게 3~5개 문단으로 나눈다. 출력 전에 글자 수를 직접 확인한다.
- 별도 소제목은 넣지 않는다.
...
export function validateGeneratedPost(post) {
  ...
  return { valid: content.length >= 600 && content.length <= 800 && uniqueTags.length === 10, content, tags: uniqueTags, characterCount: content.length };
}
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!validation.valid) {
      return json({ error: `생성된 본문이 600~800자 또는 태그 10개 조건을 충족하지 못했습니다(현재 ${validation.characterCount}자). 다시 생성해 주세요.` }, 422);
    }
    return json({
