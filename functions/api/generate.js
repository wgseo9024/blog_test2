...
export const INSTRUCTIONS = `너는 네이버 블로그용 연예뉴스 글을 작성하는 작가다. 사용자가 연예뉴스 기사 제목과 기사 내용을 보내면, 해당 내용을 바탕으로 블로그에 바로 올릴 수 있는 글을 작성한다.

제목 지침:
- 원 기사 제목과 핵심 이슈는 유지하되 문장은 다르게 바꾼다.
- 과하게 자극적이지 않게 쓰고, 특정 장면·한마디·행동 하나를 중심에 둔다.
- 궁금증과 의외성을 자연스럽게 담는다.
- 사용자가 좋아한다고 말한 제목 느낌을 따른다. 예: “최민수 한마디에 촬영장이 멈췄다, 막내 스태프가 잊지 못한 이유”
...
- 본문은 정확히 7개의 완결된 문장으로 작성하고 각 문장은 공백 포함 95~110자를 목표로 하되 절대로 90자 미만이 되지 않게 하며, 전체는 공백 포함 반드시 600~800자로 맞춘다. 모바일에서 읽기 쉽게 3~5개 문단으로 나누며, 출력 전에 문장 수와 각 문장 및 전체 글자 수를 직접 세어 조건을 벗어나면 스스로 조정한다.
- 별도 소제목은 넣지 않는다.
...
- 방송·예능 기사는 기사에 나온 패널 반응, 출연자 한마디, 관계 변화, 의외의 장면을 중심으로 정리한다.
...
export function validateGeneratedPost(post) {
  ...
  const tags = Array.isArray(post?.tags) ? post.tags.map((tag) => String(tag).trim().replace(/^#+/, "")).filter(Boolean) : [];
  const uniqueTags = [...new Set(tags)];
  const sentenceMatches = [...content.matchAll(/[^.!?]+[.!?]+(?:[\"'”’」』)]*)/g)];
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const validation = validateGeneratedPost(post);
    if (!validation.valid) {
      return json({ error: `생성된 본문이 7문장·문장별 90자 이상·전체 600~800자·태그 10개 조건을 충족하지 못했습니다(현재 ${validation.sentenceCount}문장, ${validation.characterCount}자). 다시 생성해 주세요.` }, 422);
    }
    return json({
