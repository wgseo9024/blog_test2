...
export async function onRequestPut({ request, env, params }) {
  ...
  if (action === "approve" && body.rights_confirmed !== true) return fail("권한 확인 체크와 승인 동작이 모두 필요합니다.", 422);
  ...
  const row = await env.DB.prepare(`UPDATE article_images SET rights_status=?,rights_confirmed=?,approved_for_use=?,processing_status=CASE WHEN ?=1 THEN 'ready' ELSE processing_status END,exclude_reason=CASE WHEN ?='reject' THEN COALESCE(?, '관리자 거절') ELSE exclude_reason END WHERE id=? RETURNING ${fields}`).bind(status, body.rights_confirmed === true ? 1 : 0, approved, approved, action, String(body.reason || "").slice(0,500) || null, id).first();
