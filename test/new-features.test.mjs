...
test("수동 생성 본문도 600~800자와 태그 10개를 강제한다",()=>{const tags=Array.from({length:10},(_,i)=>`태그${i}`);assert.equal(validateGeneratedPost({content:"가".repeat(600),tags}).valid,true);assert.equal(validateGeneratedPost({content:"가".repeat(599),tags}).valid,false);});
