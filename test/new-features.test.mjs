...
test("다중 출처 유사 기사는 함께 묶고 나머지는 단일 이슈 그룹으로 보존한다",()=>{const settings={similarity_threshold:.56,token_weight:.5,entity_weight:.35,time_weight:.15,max_time_gap_hours:72};const now="2026-07-11T00:00:00Z";const groups=buildGroups([{id:1,title:"리센느 새 앨범 프리티 걸 공개",source:"A",published_at:now},{id:2,title:"리센느 새 앨범 프리티 걸 공개",source:"B",published_at:now},{id:3,title:"송혜교 새 소속사 이적",source:"A",published_at:now}],settings);assert.equal(groups.length,2);assert.deepEqual(groups.map(group=>group.length).sort(),[1,2]);assert.deepEqual(new Set(groups.flatMap(group=>group.map(({article})=>article.id))),new Set([1,2,3]));});
